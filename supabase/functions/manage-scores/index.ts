...
serve(async (req) => {
  ...
  try {
    ...
    console.log('Managing scores for cadet:', { cadetId, category, points, description })
    ...
    if (currentScores) {
      ...
      if (updateError) {
        ...
        throw new Error(`Ошибка обновления баллов: ${updateError.message}`)
      }
    } else {
      ...
      if (insertError) {
        ...
        throw new Error(`Ошибка создания баллов: ${insertError.message}`)
      }
    }

    // 5. Обновляем общий счет кадета
    const { error: cadetUpdateError } = await supabaseAdmin
      .from('cadets')
      .update({
    ...
    if (cadetUpdateError) {
      console.error('Error updating cadet total score:', cadetUpdateError)
      throw new Error(`Ошибка обновления общего счета кадета: ${cadetUpdateError.message}`)
    }

    console.log('Scores managed successfully')
