...
serve(async (req) => {
  ...
  try {
    ...
    if (!name || !email || !password || !roleIds || !Array.isArray(roleIds) || roleIds.length === 0) {
      return new Response(
    ...
    console.log('Creating admin:', { name, email, roleIds })
    ...
    try {
      ...
      // 4. Получаем полные данные созданного администратора
