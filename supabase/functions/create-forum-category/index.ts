...
serve(async (req) => {
  ...
  try {
    ...
    const { name, description, icon, color, sort_order, is_active } = await req.json()
    ...
    console.log('Creating forum category:', { name, description, icon, color, sort_order, is_active })
    ...
    console.log('Forum category created successfully:', data)

    return new Response(
      JSON.stringify({ 
        success: true, 
        message: 'Категория форума успешно создана',
        category: data
      }),
      { 
        status: 200, 
